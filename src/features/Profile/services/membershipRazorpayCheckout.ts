...
export class MembershipPaymentCancelledError extends Error {
  constructor() {
    super('Payment cancelled');
    this.name = 'MembershipPaymentCancelledError';
  }
}
...
export interface OpenMembershipRazorpayParams {
  keyId: string;
  orderId: string;
  amountPaise: number;
  planName: string;
...
export function openMembershipRazorpayCheckout(
  params: OpenMembershipRazorpayParams,
): Promise<MembershipRazorpaySuccess> {
  const options = {
    key: params.keyId,
  ...
  return RazorpayCheckout.open(options)
    ...
    .catch((error: unknown) => {
      if (isPaymentCancelledError(error)) {
        throw new MembershipPaymentCancelledError();
      }
      throw error;
    });
}
