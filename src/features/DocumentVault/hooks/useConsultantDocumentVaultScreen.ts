...
import { groupVaultDocumentsByType } from '../utils/documentVaultDisplay';
import {
...
export function useConsultantDocumentVaultScreen(): UseConsultantDocumentVaultScreenResult {
  ...
  const shareTargets = useMemo(
    (): VaultShareTargetUser[] =>
      shareTargetsRaw.filter((t): t is VaultShareTargetUser => t.userType === 'user'),
    [shareTargetsRaw],
  );
  ...
  const submitShare = useCallback(async (): Promise<void> => {
    ...
    setIsBusy(true);
    try {
      await createVaultDocumentShare({ userDocumentId, targetUserId }).unwrap();
      closeShareModal();
