...
export class WalletPaymentCancelledError extends Error {
  constructor() {
    super('Payment cancelled');
    this.name = 'WalletPaymentCancelledError';
  }
}
...
export interface OpenWalletTopupRazorpayParams {
  keyId: string;
  orderId: string;
  amountPaise: number;
  customerName: string;
...
export interface WalletTopupRazorpaySuccess {
  razorpay_payment_id: string;
  razorpay_order_id: string;
}

export function openWalletTopupRazorpayCheckout(
  params: OpenWalletTopupRazorpayParams,
): Promise<WalletTopupRazorpaySuccess> {
  const options = {
    key: params.keyId,
  ...
  return RazorpayCheckout.open(options)
    ...
    .catch((error: unknown) => {
      if (isPaymentCancelledError(error)) {
        throw new WalletPaymentCancelledError();
      }
      throw error;
    });
}
